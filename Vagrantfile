...
Vagrant.configure("2") do |config|
  ...
  config.vm.network :forwarded_port,
                    guest:ENV['VM_GUEST'], host:ENV['VM_HOST']
  config.vm.network :forwarded_port,
                    guest:5000, host:1335
  config.vm.network :forwarded_port,
                    guest:8888, host:1336
  config.vm.synced_folder ".", "/vagrant", disabled:true
  config.vm.synced_folder ".", ENV['VM_PATH']
  config.vm.provision :shell, # common
                      privileged:true, path:ENV['VM_BOOTSTRAP'],
                      :args => "-c"
  ...
  config.vm.provision :shell, # dvm
                      privileged:false, path:ENV['VM_BOOTSTRAP'],
                      :args => "-b"
