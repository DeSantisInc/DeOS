...
Vagrant.configure('2') do |config|
  ...
  config.vm.provision :shell, path:ENV['VM_BOOT'], :args => '-c' # common
  config.vm.provision :unix_reboot
  config.vm.provision :shell, path:ENV['VM_BOOT'], :args => '-v' # zerotier
